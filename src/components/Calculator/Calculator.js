...
class Calculator extends Component {
    ...
    onButtonPress = (event) => {
        ...
        if (pressedButtonValue === 'C') {
            ...
            /* If the user has presseda button with a value between 0 and 9 or the period, add that value to the equation */
            equation += pressedButtonValue;
        } else if (['+', '-', '*', '/', '%'].indexOf(pressedButtonValue) !== -1) {
            /* If the user has pressed a button with a value of an operator, add the operator to the equation with a space on either side */
            equation += ` ${pressedButtonValue} `;
            ...
            try {
                ...
                console.log(error);
                alert('an error occurred, check to make sure your equation is correct');
            }
